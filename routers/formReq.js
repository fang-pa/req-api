...
const line = require("../middleware/notify");
// Setup
let date = Date.now()
let count = 0
//
        ...
        if (docInfo.titleID == 'I') {
            if (req.user.role == 'Teacher') {
                if (docInfo.status == 'yes') {
                    ...
                    if (studentInfo != undefined && studentInfo.lineToken.thereIs) {
                        await line.notify('คำร้องไม่ได้รับการอนุมัติจากผู้สอน',studentInfo.lineToken.token)
                    }
                    return res.status(200).json()
                }
                ...
                if (docInfo.status == 'yes') {
                    ...
                    if (studentInfo != undefined && studentInfo.lineToken.thereIs) {
                        await line.notify('คำร้องไม่ได้รับการอนุมัติจากผู้อำนวยการ',studentInfo.lineToken.token)
                    }
                    return res.status(200).json()
                }
                ...
                if (docInfo.status == 'yes') {
                    ...
                    if (studentInfo != undefined && studentInfo.lineToken.thereIs) {
                        await line.notify('คำร้องไม่ได้รับการอนุมัติจากนายทะเบียน',studentInfo.lineToken.token)
                    }
                    return res.status(200).json()
                }
            ...
            if (req.user.role == 'Teacher') {
                ...
                if (check.requireInfo.sub_teach.length == 1) {
                    ...
                    for (let i = 0; i < check.requireInfo.sub_teach.length; i++) {
